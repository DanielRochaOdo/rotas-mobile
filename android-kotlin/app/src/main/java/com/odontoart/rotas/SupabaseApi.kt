...
class SupabaseApi(
    ...
    suspend fun signIn(email: String, password: String): UserSession = withContext(Dispatchers.IO) {
        ...
        UserSession(
            accessToken = responseJson.stringOrNull("access_token")
                ?: throw IllegalStateException("Login sem access token."),
            refreshToken = responseJson.stringOrNull("refresh_token")
                ?: throw IllegalStateException("Login sem refresh token."),
            userId = userJson.stringOrNull("id")
    ...
    suspend fun fetchProfile(session: UserSession): UserProfile = withContext(Dispatchers.IO) {
        ensureConfigured()
        val fullSelect = "id,user_id,role,display_name,nome,is_inactive,can_access_pre_cadastro,can_access_next_route_dashboard,force_reauth_after,supervisor_id,vendedor_id,created_at"
        val fallbackSelect = "id,user_id,role,display_name,nome,is_inactive,can_access_pre_cadastro,can_access_next_route_dashboard,supervisor_id,vendedor_id,created_at"

        val row = try {
            fetchSingleRow(session, "profiles", mapOf("select" to fullSelect, "user_id" to "eq.${session.userId}", "limit" to "1"))
        } catch (error: IllegalStateException) {
            if (!error.message.orEmpty().contains("force_reauth_after", ignoreCase = true)) throw error
            fetchSingleRow(session, "profiles", mapOf("select" to fallbackSelect, "user_id" to "eq.${session.userId}", "limit" to "1"))
        }
    ...
    suspend fun createRoute(session: UserSession, name: String, date: String?): RouteItem = withContext(Dispatchers.IO) {
        ...
        val rows = writeRows(session, "routes", "POST", payload, preferRepresentation = true)
        val row = rows.optJSONObject(0) ?: throw IllegalStateException("Nao foi possivel criar a rota.")
        RouteItem(
            id = row.stringOrNull("id") ?: throw IllegalStateException("Rota criada sem id."),
    ...
    suspend fun fetchVisits(session: UserSession, role: UserRole?, limit: Int = 500): List<VisitItem> = withContext(Dispatchers.IO) {
        ...
        val rows = getRows(session, "visits", query)
        rows.mapObjects { row ->
            VisitItem(
                id = row.stringOrNull("id") ?: return@mapObjects null,
                clienteId = row.stringOrNull("cliente_id"),
    ...
    suspend fun registerNoVisit(session: UserSession, visitId: String, reason: String, observation: String?) = withContext(Dispatchers.IO) {
        patchRows(
            ...
            JSONObject()
                ...
                .put("no_visit_reason", reason)
                .put("no_visit_observation", observation?.takeIf { it.isNotBlank() } ?: JSONObject.NULL),
        )
    }
    ...
    suspend fun fetchClients(session: UserSession, search: String = "", limit: Int = 150): List<ClienteListItem> = withContext(Dispatchers.IO) {
        ...
        if (term.isNotBlank()) {
            query["or"] = "(codigo.ilike.*$term*,empresa.ilike.*$term*,nome_fantasia.ilike.*$term*,cnpj.ilike.*$term*)"
        }
        val rows = getRows(session, "clientes", query)
        rows.mapObjects { row ->
            ClienteListItem(
                id = row.stringOrNull("id") ?: return@mapObjects null,
    ...
    suspend fun createClient(session: UserSession, payload: JSONObject): ClienteListItem = withContext(Dispatchers.IO) {
        val rows = writeRows(session, "clientes", "POST", payload, preferRepresentation = true)
        val id = rows.optJSONObject(0)?.stringOrNull("id") ?: throw IllegalStateException("Empresa criada sem id.")
        fetchClients(session, payload.optString("codigo"), 20).firstOrNull { it.id == id }
            ?: throw IllegalStateException("Nao foi possivel recarregar a empresa criada.")
    }
    ...
    suspend fun fetchVendorAcceptanceDates(session: UserSession): Pair<List<String>, List<DigitalAcceptanceItem>> = withContext(Dispatchers.IO) {
        val visits = getRows(
            ...
            "visits",
            mapOf(
                "select" to "visit_date",
                "assigned_to_user_id" to "eq.${session.userId}",
                "completed_at" to "not.is.null",
                "no_visit_reason" to "is.null",
                "order" to "visit_date.asc",
                "limit" to "1000",
            ),
        )
        val requiredDates = visits.mapObjects { it.stringOrNull("visit_date") }.distinct().sorted()
        val acceptances = fetchAcceptanceRows(session, mapOf("vendor_user_id" to "eq.${session.userId}", "order" to "entry_date.desc"))
        requiredDates to acceptances
    }
    ...
    suspend fun registerDigitalAcceptance(session: UserSession, vendorName: String?, date: String, vidas: Int) = withContext(Dispatchers.IO) {
        val payload = JSONObject()
            .put("vendor_user_id", session.userId)
            .put("vendor_name", vendorName ?: JSONObject.NULL)
            .put("entry_date", date)
            .put("vidas", vidas)
            .put("created_by", session.userId)
        writeRows(session, "aceite_digital", "POST", payload, preferRepresentation = false)
    }

    suspend fun fetchQueueControls(session: UserSession): List<QueueControlItem> = withContext(Dispatchers.IO) {
        val rows = getRows(
            session,
            "queue_release_controls_view",
            mapOf(
                "select" to "empresa_id,codigo,empresa,cnpj,data_contrato,waiting_days_snapshot,eligible_at,state,effective_state,manual_block_until,manual_reason,days_remaining,updated_at",
        ...
        rows.mapObjects { row ->
            QueueControlItem(
                ...
                empresa = row.stringOrNull("empresa"),
                cnpj = row.stringOrNull("cnpj"),
                dataContrato = row.stringOrNull("data_contrato"),
                waitingDaysSnapshot = row.optIntOrNull("waiting_days_snapshot"),
                eligibleAt = row.stringOrNull("eligible_at"),
                state = row.stringOrNull("state") ?: "PENDING_WAIT",
                effectiveState = row.stringOrNull("effective_state") ?: "PENDING_WAIT",
                manualBlockUntil = row.stringOrNull("manual_block_until"),
                manualReason = row.stringOrNull("manual_reason"),
                daysRemaining = row.optIntOrNull("days_remaining"),
                updatedAt = row.stringOrNull("updated_at"),
            )
        }
    }

    suspend fun applyQueueAction(
        session: UserSession,
        empresaId: String,
        action: String,
        ...
        rpc(
            session,
            "queue_release_apply_action",
            JSONObject()
                .put("p_empresa_id", empresaId)
                .put("p_action", action)
                .put("p_waiting_days", waitingDays ?: JSONObject.NULL)
                .put("p_block_days", blockDays ?: JSONObject.NULL)
    ...
    suspend fun fetchKpiSnapshots(session: UserSession, periodDays: Int = 30): List<KpiSnapshotItem> = withContext(Dispatchers.IO) {
        val rows = getRows(
            ...
            "kpi_sync_snapshots",
            mapOf(
                "select" to "id,codigo,empresa,categoria,vidas_qtde,status,snapshot_at,delta,vendas_qtde,cancelamentos_qtde",
                "period_days" to "eq.$periodDays",
                "order" to "snapshot_at.desc",
                "limit" to "1000",
            ),
        )
        ...
        rows.mapObjects { row ->
            ...
            if (byCode.containsKey(code)) return@mapObjects null
            val item = KpiSnapshotItem(
                ...
                snapshotAt = row.stringOrNull("snapshot_at"),
                delta = row.optIntOrNull("delta") ?: 0,
                vendasQtde = row.optIntOrNull("vendas_qtde") ?: 0,
                cancelamentosQtde = row.optIntOrNull("cancelamentos_qtde") ?: 0,
            )
            byCode[code] = item
            item
        }
    ...
    suspend fun fetchSystemNews(session: UserSession, role: UserRole?): List<SystemNewsItem> = withContext(Dispatchers.IO) {
        val query = linkedMapOf(
            ...
            "ativo" to "eq.true",
            "order" to "data_publicacao.desc,created_at.desc",
            "limit" to "100",
        )
        if (role != null) query["roles_permitidos"] = "cs.{${role.name}}"
        val rows = getRows(session, "system_news", query)
        rows.mapObjects { row ->
            SystemNewsItem(
                id = row.stringOrNull("id") ?: return@mapObjects null,
                titulo = row.stringOrNull("titulo") ?: "Sem titulo",
                descricao = row.stringOrNull("descricao") ?: "",
                tipo = row.stringOrNull("tipo") ?: "AVISO",
                modulo = row.stringOrNull("modulo") ?: "Geral",
                rolesPermitidos = row.optJSONArray("roles_permitidos")?.toStringList().orEmpty(),
                dataPublicacao = row.stringOrNull("data_publicacao") ?: "",
                ativo = row.optBooleanSafe("ativo", defaultValue = true),
            )
        }
    }
    ...
    suspend fun fetchAuditLogs(session: UserSession, action: String? = null, table: String? = null): List<AuditLogItem> = withContext(Dispatchers.IO) {
        val query = linkedMapOf(
            "select" to "id,table_name,action,record_id,user_id,user_name,old_data,new_data,created_at",
            "order" to "created_at.desc",
            "limit" to "200",
        )
        if (!action.isNullOrBlank() && action != "all") query["action"] = "eq.$action"
        if (!table.isNullOrBlank() && table != "all") query["table_name"] = "eq.$table"
        val rows = getRows(session, "audit_logs", query)
        rows.mapObjects { row ->
            AuditLogItem(
                id = row.stringOrNull("id") ?: return@mapObjects null,
                tableName = row.stringOrNull("table_name") ?: "-",
                ...
                oldData = row.optJSONObject("old_data")?.toString(2),
                newData = row.optJSONObject("new_data")?.toString(2),
                createdAt = row.stringOrNull("created_at") ?: "",
            )
        }
    }
    ...
    suspend fun fetchManagedProfiles(session: UserSession): List<ManagedProfileItem> = withContext(Dispatchers.IO) {
        val rows = getRows(
            ...
            "profiles",
            mapOf(
                "select" to "id,user_id,role,display_name,nome,can_access_pre_cadastro,can_access_next_route_dashboard,supervisor_id,vendedor_id,is_inactive",
                "order" to "display_name.asc.nullslast",
                "limit" to "500",
            ),
        )
        rows.mapObjects { row ->
    ...
    private suspend fun fetchAcceptanceRows(session: UserSession, extra: Map<String, String>): List<DigitalAcceptanceItem> {
        val query = linkedMapOf("select" to "id,vendor_user_id,vendor_name,entry_date,vidas")
        query.putAll(extra)
        val rows = getRows(session, "aceite_digital", query)
        return rows.mapObjects { row ->
            DigitalAcceptanceItem(
                id = row.stringOrNull("id") ?: return@mapObjects null,
                vendorUserId = row.stringOrNull("vendor_user_id"),
                vendorName = row.stringOrNull("vendor_name"),
                entryDate = row.stringOrNull("entry_date") ?: return@mapObjects null,
                vidas = row.optIntOrNull("vidas") ?: 0,
            )
        }
    }

    private fun parseCliente(value: Any?): ClienteInfo? = when (value) {
        is JSONObject -> {
    ...
    private fun fetchSingleRow(session: UserSession, table: String, query: Map<String, String>): JSONObject? {
        val rows = getRowsBlocking(session, table, query)
    ...
    private fun getRowsBlocking(session: UserSession, table: String, query: Map<String, String>): JSONArray {
        ensureConfigured()
        val request = authorizedRequestBuilder(restUrl(table, query), session.accessToken).get().build()
    ...
    private fun patchRows(session: UserSession, table: String, query: Map<String, String>, payload: JSONObject) {
        writeRows(session, table, "PATCH", payload, preferRepresentation = false, query = query)
    }

    private fun deleteRows(session: UserSession, table: String, query: Map<String, String>) {
        val request = authorizedRequestBuilder(restUrl(table, query), session.accessToken).delete().build()
        execute(request)
    }

    private fun rpc(session: UserSession, function: String, payload: JSONObject): String {
        ensureConfigured()
        val request = authorizedRequestBuilder("$supabaseUrl/rest/v1/rpc/$function", session.accessToken)
            .post(payload.toString().toRequestBody(JSON_MEDIA_TYPE))
            .build()
    ...
    private fun execute(request: Request): String {
        httpClient.newCall(request).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                throw IllegalStateException("Falha na API (${response.code}): ${extractServerError(body)}")
            }
            return body
        }
    }
    ...
    private fun ensureConfigured() {
        if (!isConfigured()) throw IllegalStateException("SUPABASE_URL/SUPABASE_ANON_KEY nao configurados no Android.")
    }
